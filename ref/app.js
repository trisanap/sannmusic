...
var app = (function() {
  ...
  function switchTab(tabId) {
    ...
    dom.breadcrumb.style.display = tabId === 'folders' ? '' : 'none';
    ...
    if (tabId === 'playlists') {
      ...
      if (state.items.length === 0) loadDirectory(currentDir());
      else renderItems();
    }
  }
  ...
  function renderFavorites() {
    ...
    for (var i = 0; i < state.likedTracks.length; i++) {
      ...
      html += '<span class="item-album">' + escapeHtml(track.album || '') + '</span>';
  ...
  function setupHeaderScroll() {
    dom.mainContent.addEventListener('scroll', function() {
      if (dom.mainContent.scrollTop > 20) {
  ...
  function updateHeaderChrome() {
    var onFolders = state.activeTab === 'folders';
    var hasBack = dom.headerBack.style.display !== 'none';
    dom.header.style.display = (onFolders || hasBack) ? '' : 'none';
  }

  function showHeaderBack(onClick) {
    dom.headerBack.style.display = '';
    dom.headerBack.onclick = function() {
      hideHeaderBack();
      onClick();
    };
    updateHeaderChrome();
  ...
  function hideHeaderBack() {
    dom.headerBack.style.display = 'none';
    dom.headerBack.onclick = null;
    updateHeaderChrome();
  }
  ...
  function renderItems() {
    ...
    if (atRoot) dom.header.style.display = ''; // restore header at root
    var html = '';
    ...
    if (!atRoot && audioFiles.length > 0) {
      ...
      // Hide main header bar on detail views
...
function loadPlaylists() {
  ...
  function init() {
    ...
    // Breadcrumb hidden on startup (Home is default tab, not Folders)
